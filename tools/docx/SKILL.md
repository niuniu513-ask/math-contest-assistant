...
Markdown 输入中的行内 `\(...\)` 与独立 `\[...\]` 会在交给 Pandoc 前自动归一化为 `$...$`/`$$...$$`（代码围栏内不处理），确保 DOCX 输出为可编辑 OMML；LaTeX 源码侧仍遵循“独立公式用 `\[...\]`、禁用 `$$...$$`”的规范。
...
python scripts/paper_format.py validate "<PROJECT_ROOT>/完整论文.docx" --contest cumcm --rendered-pages <PDF总页数> --body-pages <不含附件的主体页数> --min-equations 15 --min-figures 12 --min-references 8
python scripts/equations.py verify-conversion "<PROJECT_ROOT>/完整论文.docx"
```

`verify-conversion` 仅适用于由本工具的 Pandoc 转换生成、带 `.conversion.json` 的 DOCX；直接由 `paper_format.py` 构建时省略。`paper_format.py validate` 输出结构化指标并检查官方前置结构、图表编号与正文引用、参考文献双向对应及实际页数。主体不设默认最低页数；当届规则或用户明确给出阈值时才传入 `--target-body-pages`。完整竞赛论文使用本 Skill 的深度质量档：至少 15 个可编辑公式、12 幅非装饰性图和 8 条真实领域文献；精简稿或竞赛上限冲突时按用户/官方规则显式覆盖。数量检查不能替代 `paper_content_audit.py` 的推导、图表解读和竞赛语境审计。结构校验后，把 DOCX 渲染成 PDF 或图片逐页检查分页、公式、表格、图片、页眉页脚和字体替换。
