...
def validate_paper_structure(
    ...
    *,
    ...
    min_references = 0 if min_references is None else min_references
    ...
    reference_heading = next(
        (index for index, value in enumerate(texts) if value.strip().lower() in {"参考文献", "references"}),
        None,
    )
    references = 0
    if reference_heading is not None:
        references = sum(bool(re.match(r"^\s*\[(\d+)\]", value)) for value in texts[reference_heading + 1:])
    ...
    if references < min_references:
        errors.append(f"预警：仅检测到 {references} 条参考文献，低于质量目标 {min_references}")
    ...
    if rendered_pages is None and require_rendered_pages and target_body_pages is not None:
        errors.append(
            f"预警：未提供渲染页数，无法检查正文 {target_body_pages} 页质量目标和 "
            f"{official_max_pages} 页官方上限"
        )
    elif rendered_pages is not None:
        if body_pages is None and target_body_pages is not None:
            errors.append("未提供正文页数，不能排除附件后执行正文页数门禁")
        elif body_pages is not None and body_pages > rendered_pages:
            errors.append(f"正文页数 {body_pages} 不能大于渲染总页数 {rendered_pages}")
        elif target_body_pages is not None and body_pages < target_body_pages:
            errors.append(
                f"正文 {body_pages} 页，低于 {target_body_pages} 页质量目标；附件不计入正文"
            )
        if official_max_pages is not None and body_pages is not None and body_pages > official_max_pages:
            errors.append(
                f"正文 {body_pages} 页，超过当前核验的官方上限 {official_max_pages} 页"
            )
    return errors
...
def validate_document(
    ...
    *,
    ...
    issues = validate_paper_structure(
        ...
        target_body_pages=target_body_pages,
    ...
    return {
        ...
        "metrics": {
            ...
            "body_pages": body_pages,
            "appendix_pages": (
                rendered_pages - body_pages
                if rendered_pages is not None and body_pages is not None
                else None
            ),
            "equations": len(doc._element.findall(f".//{qn('m:oMath')}")),
            "figures": len(doc._element.findall(f".//{qn('a:blip')}")),
            "tables": len(doc.tables),
            "references": sum(
                bool(re.match(r"^\s*\[(\d+)\]", value))
                for value in _document_texts(doc)
...
def main():
    ...
    validate.add_argument("--body-pages", type=int)
    validate.add_argument("--target-body-pages", type=int)
    validate.add_argument("--min-equations", type=int)
    validate.add_argument("--min-figures", type=int)
    validate.add_argument("--min-references", type=int)
    ...
    result = validate_document(
        ...
        target_body_pages=arguments.target_body_pages,
        min_equations=arguments.min_equations,
        min_figures=arguments.min_figures,
        min_references=arguments.min_references,
    )
