...
def inspect_paper(
    ...
    *,
        ...
        try:
            ...
            if contest == "cumcm":
                ...
                if body_start > rendered_pages + 1:
                    ...
                    main_matter_pages = (appendix_start - 1) if appendix_start else rendered_pages
                    appendix_pages = rendered_pages - main_matter_pages
    ...
    if minimum_pages:
        if contest == "cumcm":
            if main_matter_pages is None:
                issues.append("无法确定论文主体页数，不能执行主体最低页数质量门禁")
            elif main_matter_pages < minimum_pages:
                issues.append(
                    f"论文主体页数 {main_matter_pages}，低于质量目标 {minimum_pages}；附件不计入主体"
                )
        elif rendered_pages is not None and rendered_pages < minimum_pages:
            issues.append(f"PDF 总页数 {rendered_pages}，低于质量目标 {minimum_pages}")
