...
主体不设默认最低页数。只有当届官方规则或用户明确提出最低篇幅时才传入 `--min-pages`；对 CUMCM，该参数按 PDF 开始至附件前一页的论文主体检查，`body_pages` 仍保留用于可能存在的官方正文上限口径。字词、公式、图和表的数量也不设永久定额，按证据计划显式传入。不得为页数或数量目标重复内容、制造空白或加入无主张支撑的图表。
