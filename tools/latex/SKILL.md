...
CUMCM 内部质量档：正文（不含附录）20~25 页、公式 ≥15、非装饰图 ≥12。`--quality-checks` 默认按 `min_pages=20`、`max_pages=25`、`min_equations=15`、`min_figures=12` 检查，低于下限或超过上限都阻断发布；官方 2026 上限为 30 页，确需超过 25 页时必须显式传 `--max-pages 30` 并附 `--override-reason`。MCM/ICM 与 generic 不套用国赛口径，页数上限按当届官方规则显式传入。对 CUMCM，`min_pages`/`max_pages` 按 PDF 开始至附件前一页的论文主体检查；不得为页数或数量目标重复内容、制造空白或加入无主张支撑的图表。
