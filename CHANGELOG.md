...
强化使用边界、论文证据门禁、可选协作入口和论文格式默认值。

- **docs**：新增根目录 `使用指南.md`，明确 Skill 生成物是草稿/参考材料，补充人工核对责任和默认交付格式。
- **feat**：将 Claim–Evidence Matrix 设为 `W1` 硬性门槛，要求摘要、结论和正文关键主张逐行绑定公式、表、图、代码输出或验证记录。
- **docs**：在主 `SKILL.md`、`README.md`、`Subagent调度.md` 中加入可选协作菜单，规则核验、附件盘点、文献调研、算法原型、独立实验、双语言对照和术语核验默认关闭。
- **change**：论文默认交付改为 Word `完整论文.docx`；LaTeX/PDF 仅在用户显式要求时生成，双格式继续共用同一证据源。

## [1.6.1] - 2026-08-19

修复 Windows 环境下论文正文及相关命令输出的中文乱码。

- **fix**：旧版 Markdown→DOCX 入口统一使用 UTF-8 标准输出，并兼容 UTF-8 BOM 与 GB18030 编码的 Markdown 草稿。
- **fix**：DOCX 正文、摘要关键词和代码块显式设置 `ascii`、`hAnsi`、`eastAsia` 字体槽，避免 Word 按系统代码页错误替换中文。
- **test**：新增中文特殊符号、字体映射和 GB18030 草稿回归测试；全套 30 项测试通过。
...
- **fix**：删除 1.4.6/1.4.7 新增的“中文题图内文字必须中文/可读中英文一致/禁止变量名列名直接当图例”等规则，与 v1.3 一致——图表文字语言不再设红线。
- **fix**：保留中文字体动态选择（v1.3 `plot_style.choose_font` 同款逻辑）与“最终视觉检查”中的缺字/乱码/方框检查，作为防乱码的实际保障。
- **docs**：`code-generation-guide.md`、`可视化规范.md`、`图表选择与避坑.md`、`论文自审框架.md` 同步回退；不再要求 BLOCKED，只要求导出前检查缺字。
...
- 新增输入预读与摘要缓存，减少重复解析原始附件。
- 新增统一 Python UTF-8 配置、matplotlib 绘图模板和 C 题图表演化合同。
- 新增按问题隔离求解目录、先算后画、图表来源登记和重复图检查。
- 新增运行效率与止损规则、单命令超时和恢复点命令。
- 新增摘要一页、推导链、写作分节和 LaTeX 表格/编译审计。
- 主入口接入输入预读、写作规范、图表演化规范和运行效率规范。
