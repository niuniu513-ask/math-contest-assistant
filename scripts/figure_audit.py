...
def audit_figure_directory(
    ...
    *,
    ...
    if require_categories and min_per_category < 1:
    ...
    if not by_stem:
        issues.append({"severity": "FAIL", "message": "figures 目录中没有可审计的 SVG 或 PNG"})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="检查 SVG/PNG 配对、可编辑文本和 PNG DPI；按需检查三类图")
    parser.add_argument("figures_dir", help="PROJECT_ROOT 下的 figures 目录")
    parser.add_argument("--min-dpi", type=int, default=300)
    parser.add_argument("--min-per-category", type=int, default=1, help="启用类别检查时每类最低数量")
    parser.add_argument(
        "--questions",
        nargs="+",
        default=(),
    ...
    category.add_argument("--require-categories", action="store_true", help="要求 raw/process/result 三类前缀和子问题覆盖")
    category.add_argument("--no-category-check", action="store_true", help="兼容旧命令；类别检查默认关闭")
