...
- 2026 年官方《全国大学生数学建模竞赛论文格式规范》规定摘要原则上不超过一页、正文不超过 30 页，没有规定最低页数。
- 本 Skill 设国赛正文内部质量档 20~25 页（非官方硬性）：`paper_format.py validate` 传 `--target-body-pages 20 --official-max-pages 30`，正文低于 20 页阻断，超过 25 页需人工复核并在门禁记录理由。
- 篇幅完整性通过题目覆盖、模型推导、结果、验证、讨论、图表和引用检查判断。
...
该检查会报告篇幅、公式/图/表数量、图表编号与正文引用、参考文献双向对应以及页数问题，但不设置默认最低数量。2026 年 CUMCM 正文 30 页上限属于该届已核验的官方硬约束；使用其他届次或竞赛时必须重新核验并显式传入相应上限。OOXML 校验通过后仍需渲染抽检，因为 XML 合法不等于版面正确。
